...
function getFormattedDate() {
  ...
  const month = (currentDate.getMonth() + 1).toString().padStart(2, '0'); // Adding 1 to the month because it's 0-based
...
var current_date = getFormattedDateTime()
var dt_date = getFormattedDate()
// console.log(formattedDate); // Output will be in 'YYYYMMDD' format
...
app.post('/broker-signals', async (req, res) => {
  ...
  try {
        ...
        if (client_key != undefined) {
          ...
          if (FIRST3_KEY == process.env.PANEL_FIRST_THREE) {
            ...
            if (segment == 'C' || segment == 'c') {
              ...
              EXCHANGE = "CDS";
            } else if (segment == 'CO' || segment == 'co') {
              instrument_query = { symbol: input_symbol, segment: "CO", expiry: expiry, strike: strike, option_type: Trade_Option_Type }
              EXCHANGE = "CDS";
              trade_symbol = input_symbol + day_expiry + ex_day_expiry + ex_year_expiry + strike + Trade_Option_Type;
              findSignal = { entry_type: "LE", dt_date: dt_date, symbol: input_symbol, expiry: expiry, option_type: option_type, segment: segment, strategy: strategy, entry_type: type === "LE" || type === "LX" ? 'LE' : type === "SE" || type === "SX" ? "SE" : "LE" }

            }
            console.log("instrument_query =>", instrument_query);
            ...
            console.log('RECEIVED_SIGNALS_TOKEN ' + token);
            ...
            if (process.env.PANEL_KEY == client_key) {
              ...
              try {
                ...
                console.log("All documents:", AliceBluedocuments.length);
    ...
    console.log("Broker siganl-", error);
  }
})
