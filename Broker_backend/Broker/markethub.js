...
function transformTradeSymbol(tradeSymbol) {
  ...
  for (const month of months) {
    if (tradeSymbol.includes(month)) {
      // Find the position of the month in the tradeSymbol
      const monthIndex = tradeSymbol.indexOf(month);

      // Extract the part before the month, removing the two digits before the month
      const beforeMonth = tradeSymbol.slice(0, monthIndex - 2);

      // Extract the first two digits after the month
      const newDigits = tradeSymbol.slice(monthIndex + 3, monthIndex + 5);

      // Extract the part after the month and digits
      const afterDigits = tradeSymbol.slice(monthIndex + 5);

      // Construct the new tradeSymbol
      return `${beforeMonth}${newDigits}${month}${afterDigits}`;
    }
  }

  // Return the original tradeSymbol if no month is found
  return tradeSymbol;
}
      ...
      if (type == "LE" || type == "SE") {
        const requestPromises = AllClientData.map(async (item) => {
          if (segment.toUpperCase() != "C") {
            ...
            item.postdata.tradingsymbol = transformTradeSymbol(
              token[0].tradesymbol
            );
          ...
          axios(config)
            .then(async (response) => {
              if (response.data.status == "success") {
                if (response.data.data.length > 0) {
                  ...
                  const Exist_entry_order = response.data.data.find(
                    (item1) => item1.token === token[0].instrument_token
                  );

                  if (Exist_entry_order != undefined) {
                    const possition_qty =
                      parseInt(Exist_entry_order.buy_qty) -
                      parseInt(Exist_entry_order.sell_qty);
            ...
            .catch(async (error) => {
              fs.appendFile(
                ...
                "TIME " +
                  new Date() +
                  " MARKETHUB POSITION DATA ERROR CATCH - " +
                  item.UserName +
                  " ERROR - " +
...
const EntryPlaceOrder = async (item, filePath, signals, signal_req) => {
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " MARKETHUB BEFORE PLACE ORDER USER ENTRY- " +
      item.UserName +
      " REQUEST -" +
  ...
  axios(config)
    .then(async (response) => {
      ...
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MARKETHUB AFTER PLACE ORDER USER ENTRY - " +
          item.UserName +
          " RESPONSE -" +
    ...
    .catch(async (error) => {
      ...
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MARKETHUB AFTER PLACE ORDER CATCH ENTRY - " +
          item.UserName +
          " ERROR -" +
          JSON.stringify(error) +
          "\n",
...
const ExitPlaceOrder = async (
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " MARKETHUB BEFORE PLACE ORDER USER EXIT- " +
      item.UserName +
      " REQUEST -" +
      JSON.stringify(item.postdata) +
      "\n",
  ...
  axios(config)
    .then(async (response) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MARKETHUB AFTER PLACE ORDER USER EXIT- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(response.data) +
          "\n",
    ...
    .catch(async (error) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MARKETHUB AFTER PLACE ORDER USER EXIT CATCH- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(error) +
          "\n",
