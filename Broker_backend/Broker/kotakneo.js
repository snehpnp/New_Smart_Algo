    ...
    if (token != 0) {
      if (type == "LE" || type == "SE") {
        if (segment.toUpperCase() != "C") {
          ...
          if (segment.toLowerCase() === "o" || segment.toLowerCase() === "fo") {
            ...
            // const command = `grep '${pattern}' ${filePath1}`;
            command = `grep -E ".*(${pattern}).*.*(bse_fo).*.*(${input_symbol}).*.*(${optionType}).*" ${filePath1}`;
          }else {
          ...
          axios(config)
            .then(async (response) => {
              if (response.data.stat == "Ok") {
                ...
                const Exist_entry_order = response.data.data.find(
                  (item1) => item1.tok === token[0].instrument_token
                );

                if (Exist_entry_order != undefined) {
                  item.postdata.ts = Exist_entry_order.trdSym;
                  const possition_qty =
                    parseInt(Exist_entry_order.flBuyQty) -
                    parseInt(Exist_entry_order.flSellQty);
            ...
            .catch(async (error) => {
              fs.appendFile(
                ...
                "TIME " +
                  new Date() +
                  " KOTAK NEO POSITION DATA ERROR CATCH - " +
                  item.UserName +
                  " ERROR - " +
...
const EntryPlaceOrder = async (item, filePath, signals, signal_req) => {
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " KOTAK NEO BEFORE PLACE ORDER USER ENTRY- " +
      item.UserName +
      " REQUEST -" +
  ...
  axios(config)
    .then(async (response) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " KOTAK NEO AFTER PLACE ORDER USER ENTRY - " +
          item.UserName +
          " RESPONSE -" +
    ...
    .catch(async (error) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " KOTAK NEO AFTER PLACE ORDER CATCH ENTRY - " +
          item.UserName +
          " ERROR -" +
          JSON.stringify(error) +
          "\n",
...
const ExitPlaceOrder = async (
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " KOTAK NEO BEFORE PLACE ORDER USER EXIT- " +
      item.UserName +
      " REQUEST -" +
      JSON.stringify(item.postdata) +
      "\n",
  ...
  axios(config)
    .then(async (response) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " KOTAK NEO AFTER PLACE ORDER USER EXIT- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(response.data) +
          "\n",
    ...
    .catch(async (error) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " KOTAK NEO AFTER PLACE ORDER USER EXIT CATCH- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(error) +
          "\n",
