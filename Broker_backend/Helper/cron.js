module.exports = function (app) {
  ...
  const RemoveAllTextFiles = () => {

    const directory = path.join(__dirname, "..", "AllPanelTextFile");
  ...
  const downloadAndExtractUpstox = async () => {
    ...
    try {
      ...
      const outputFolder = path.join(
        __dirname,
        "../AllInstrumentToken/upstoxinstrument"
      );
  ...
  async function downloadAndSwastika() {
    try {
      const ulrs = [
        {
          url: "https://justradeuat.swastika.co.in/NSE_symbols.txt.zip",
          filename: "NSE_symbols.txt.zip",
        },
        {
          url: "https://justradeuat.swastika.co.in/NFO_symbols.txt.zip",
          filename: "NFO_symbols.txt.zip",
        },
        {
          url: "https://justradeuat.swastika.co.in/MCX_symbols.txt.zip",
          filename: "MCX_symbols.txt.zip",
        },
        {
          url: "https://justradeuat.swastika.co.in/CDS_symbols.txt.zip",
          filename: "CDS_symbols.txt.zip",
        },
        {
          url: "https://justradeuat.swastika.co.in/BSE_symbols.txt.zip",
          filename: "BSE_symbols.txt.zip",
        },
        {
          url: "https://justradeuat.swastika.co.in/NCX_symbols.txt.zip",
          filename: "NCX_symbols.txt.zip",
        },
      ];

      const outputFolder = path.join(
        __dirname,
        "../AllInstrumentToken/swastika"
      );
      if (!fs.existsSync(outputFolder)) {
        fs.mkdirSync(outputFolder);
      }
          ...
          try {
            ...
            if (response.status !== 200) {
              return {
                ...
                status: `Failed with status code: ${response.status}`,
              };
            }
            ...
            if (!contentType.includes("application/zip")) {
              return {
                filename: item.filename,
                status: `Unexpected content type: ${contentType}`,
              };
            }
      ...
      downloadResults.forEach((result) => {
        if (result.status !== "Success") {
          console.log(
            `Download of ${result.filename} encountered an issue: ${result.status}`
          );
          return;
        } else {
          console.log(
  ...
  const downloadAndExtractICICIDirect = async () => {
    try {
      ...
      const outputFolder = path.join(
        __dirname,
        "../AllInstrumentToken/iciciinstrument"
      );
      if (!fs.existsSync(outputFolder)) {
        fs.mkdirSync(outputFolder);
      }
    ...
    try {
      const ulrs = [
        ...
        },
        {
          ...
          filename: "BFO_symbols.txt.zip",
        },

        {
          url: "https://api.shoonya.com/NCX_symbols.txt.zip",
          filename: "NCX_symbols.txt.zip",
        },
      ];
