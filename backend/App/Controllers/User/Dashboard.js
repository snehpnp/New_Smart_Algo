...
const user_fund_logs = db.user_fund_logs;
...
class Dashboard {
  ...
  async getClientServices(req, res) {
    try {
      ...
      const GetServiceStrategy = GetAllClientServices.map((item) => ({
        ...
        service_name:
          item.service.name + " [ " + item.categories.segment + " ]",
      }));
      ...
      return res.send({
        ...
        status_startegy:
          GetAllClientServices[0].userInfo.multiple_strategy_select,
        // page: Number(page),
        // limit: Number(limit),
        totalCount: totalCount,
      ...
      if (statusStartegyUser == "1") {
        ...
        if (isEmpty == false) {
          ...
          const result = Object.keys(servicesData)
            .filter(
              (key) =>
                Array.isArray(servicesData[key].strategy_id) &&
                servicesData[key].strategy_id.length === 0
            )
            .reduce((obj, key) => {
          ...
          const serviceName = matchingObject
            ? matchingObject.service_name
            : null;
          ...
          if (isEmptyStartegyArray == false) {
            return res.send({
              status: false,
              msg: "Please Select one Strategy a script " + serviceName,
              data: [],
            });
          }
        }
      ...
      for (const key in servicesData) {
        if (servicesData[key]) {
          ...
          if (matchedObject.strategy_id != undefined) {
            ...
            matchedObject.strategy_id = matchedObject.strategy_id.filter(
              (item) => item instanceof ObjectId
            );
          }

          if (matchedObject.active_status) {
            matchedObject.active_status =
              matchedObject.active_status == true ? "1" : "0";
          }
          ...
          if (
            matchedObject.active_status ||
            matchedObject.active_status == false
          ) {
          ...
          if (matchedObject.order_type) {
            var msg =
              matchedObject.order_type == "1"
                ? "MARKET"
                : matchedObject.order_type == "2"
                ? "LIMIT"
                : matchedObject.order_type == "3"
                ? "STOPLOSS LIMIT"
                : "STOPLOSS MARKET";
          ...
          if (matchedObject.product_type) {
            var msg =
              matchedObject.product_type == "1"
                ? "CNC"
                : matchedObject.product_type == "2"
                ? "MIS"
                : matchedObject.product_type == "3"
                ? "BO"
                : "CO";
            ...
            const user_activity = new user_activity_logs({
              user_id: UserData._id,
              message:
                Service_name[0].name + "  product_type " + msg + " Update",
  ...
  async TradingOff(req, res) {
    try {
      ...
      if (User_information[0].Role == "ADMIN") {
        ...
        const admin_Update = await live_price.updateOne(
          { _id: live_price1[0]._id },
          { $set: { trading_status: "off" } }
        );
  ...
  async ModifyUpdates(req, res) {
    try {
      ...
      var User_information = await User_model.find({ _id: user_id }).select(
        "web_url signals_execution_type"
      );
      ...
      if (
        User_information[0].signals_execution_type !==
        obj.signals_execution_type
      ) {
        abc["signals_execution_type"] = obj.signals_execution_type;
      }

      if (Object.keys(abc).length != 0) {
        const User_Update = await User_model.updateMany(
          { _id: User_information[0]._id },
          { $set: abc }
        );
  ...
  async USerFundUpdate(req, res) {
    try {
      const { user_id, fund_type, fund_value } = req.body;

      var User_information = await User_model.findOne({ _id: user_id });
      if (User_information.length == 0) {
        return res.send({ status: false, msg: "User Not exists", data: [] });
      }

      User_information.fund_type = fund_type;
      User_information.fund_value = fund_value;

      User_information.save();

      const UserFndLogs = new user_fund_logs({
        user_Id: User_information._id,
        fund_status: fund_type,
        fund_amount: fund_value,
      });

      await UserFndLogs.save();

      return res.send({ status: true, msg: "User Fund Update", data: [] });
    } catch (error) {
      console.log("Error ", error);
    }
  }

  async UserFundLogs(req, res) {
    try {
      const { user_id } = req.body;
      var User_information = await User_model.findOne({ _id: user_id });
      if (User_information.length == 0) {
      ...
      const UserFndLogs = await user_fund_logs.find({
        user_Id: User_information._id,
      }).sort({ createdAt: -1 });
      return res.send({
        status: true,
