    ...
    try {
      if (user_email != undefined) {
        ...
        if (Get_User.length > 0) {
          ...
          var LoginID = Get_User[0].client_code;
          var Password = Get_User[0].api_secret;
          var PancardNo = Get_User[0].app_id;
          var FactorTwo = Get_User[0].app_key;

          if (LoginID == "" || LoginID == null) {
            return res.send({
              status: false,
              msg: "Please Update CLIENT ID in Broker key...",
            });
          }
          if (PancardNo == null) {
            return res.send({
              status: false,
              msg: "Please Update Pancard  in Broker key...",
            });
          }
          if (Password == "" || Password == null) {
            return res.send({
              status: false,
              msg: "Please Update Password Code in Broker key...",
            });
          }
          if (FactorTwo == "" || FactorTwo == null) {
            return res.send({
              status: false,
              msg: "Please Update FactorTwo Code in Broker key...",
            });
          }
          ...
          await axios
            .request(config)
            .then(async (response) => {
              if (response.data.data.jwtToken != undefined) {
                let AccessToken = response.data.data.jwtToken;
                ...
                return res.send({
                  status: false,
                  msg: "Please Update correct credentials in Broker key...",
                });
              }
            })
            .catch((error) => {
    ...
    if (FindUserBrokerResponse.length > 0) {
      FindUserBrokerResponse.forEach((data1) => {
        var config = {
          method: "get",
          url:
            "http://trdapi.markethubonline.com:27005/api1/orderstatus?client_id=" +
            user_info[0].client_code +
            "&user_order_number=" +
            data1.order_id,
          headers: {
