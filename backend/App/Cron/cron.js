...
const {
  Alice_Socket,
  updateChannelAndSend,
} = require("../Helper/Alice_Socket");
...
const MainSignalsRemainToken = async () => {
  ...
  if (result.length > 0) {
    result.forEach(async (element) => {
      ...
      const update_token = await token_chain.updateOne(filter, update, {
        upsert: true,
      });
    });
  }
...
const TruncateTableTokenChainAdd_fiveMinute = async () => {
  ...
  if (
    currentHour >= 9 ||
    (currentMinute >= 15 && currentHour <= 23) ||
    currentMinute <= 30
  ) {
    ...
    if (AliceToken.length > 60000) {
      ...
      const unmatchedTokenChannel = updateTokenAfter
        .map((item) => `${item.exch}|${item._id}`)
        .join("#");
...
const Get_Option_All_Token_Chain = async () => {
  try {
    ...
    var concatenatedArray1 = concatenatedArray.substring(
      0,
      concatenatedArray.length - 1
    );
    const filter = { broker_name: "ALICE_BLUE" };
    const updateOperation = { $set: { Stock_chain: concatenatedArray1 } };
    const Update_Stock_chain = await live_price.updateOne(
      filter,
      updateOperation
    );
    return;
        ...
        resultStrike.forEach((element) => {
          ...
          result.forEach(async (element1) => {
            if (element.document.strike == element1.strike) {
              ...
              const update_token = await stock_live_price.updateOne(
                filter,
                update,
                { upsert: true }
              );
...
const TokenSymbolUpdate = async () => {
  ...
  try {
    ...
    if (response.data.length > 0) {
      const filteredDataO = response.data.filter(
        (element) =>
      ...
      const filteredDataF = response.data.filter(
        (element) =>
          (element.instrumenttype === "FUTSTK" ||
            element.instrumenttype === "FUTIDX") &&
          element.exch_seg === "NFO" &&
          element.name != ""
      );

      const filteredDataMF = response.data.filter(
        (element) => element.instrumenttype === "FUTCOM" && element.name != ""
      );
      const filteredDataMO = response.data.filter(
        (element) =>
          (element.instrumenttype === "OPTFUT" ||
            element.instrumenttype === "OPTCOM") &&
          element.name != ""
      );
      const filteredDataCO = response.data.filter(
        (element) => element.instrumenttype === "OPTCUR" && element.name != ""
      );
      const filteredDataCF = response.data.filter(
        (element) => element.instrumenttype === "FUTCUR" && element.name != ""
      );
      const filteredDataC = response.data.filter(
        (element) => element.symbol.slice(-3) === "-EQ" && element.name != ""
      );

      const filteredDataBO = response.data.filter(
        (element) =>
          (element.instrumenttype === "OPTIDX" ||
            element.instrumenttype === "OPTSTK") &&
      ...
      const filteredDataBF = response.data.filter(
        (element) =>
          (element.instrumenttype === "FUTSTK" ||
            element.instrumenttype === "FUTIDX") &&
          element.exch_seg === "BFO" &&
      ...
      const filteredDataBC = response.data.filter(
        (element) =>
          element.instrumenttype === "" &&
          element.exch_seg === "BSE" &&
          element.name != ""
      );
      ...
      for (const element of response.data) {
        try {
          ...
          if (element.token != undefined) {
            const exist_token = await Alice_token.findOne(
              { instrument_token: element.token },
              { instrument_token: 1 }
            );
            if (exist_token == null) {
    ...
    if (currentHour >= 9 && currentHour <= 24) {
      ...
      Papa.parse(data, {
        complete: async (result) => {
          ...
          const uniqueSymbols = [
            ...new Set(sheet_Data.map((item) => item.SYMBOL)),
          ];
          sheet_Data = sheet_Data.filter(
            (item, index, self) =>
