    ...
    $group: {
      ...
      total_client: {
        $sum: {
          $cond: [
            {
              $and: [{ $eq: ["$Role", "USER"] }, { $eq: ["$Is_Active", "1"] }],
            },
            1,
            0,
          ],
            ...
            $match: {
              $expr: {
                $gt: [
                  ...
                  }, // Ensure $month_ago_date exists in companies
                ],
              },
            },
          },
...
db.createView("open_position", "mainsignals", [
  ...
  },
  {
    $addFields: {
      ...
      stockInfo_bp1: {
        $ifNull: [{ $toDouble: { $arrayElemAt: ["$stockInfo.bp1", 0] } }, 0],
      },
      stockInfo_sp1: {
        $ifNull: [{ $toDouble: { $arrayElemAt: ["$stockInfo.sp1", 0] } }, 0],
      },
      stockInfo_curtime: {
...
db.createView("strategyViewNames", "usermakestrategies", [
  ...
  },
  {
    $addFields: {
      viewName: {
        $concat: ["M", "$timeframe", "_", "$tokensymbol", "_make_", "$name"],
      },
    },
  },
  {
...
db.createView("Cilents_service_stg", "users", [
  {
    $match: {
      $and: [{ Role: "USER", Is_Active: "1", EndDate: { $gte: new Date() } }],
    },
  },
...
db.createView("choiceView", "users", [
  {
    $match: {
      broker: "28",
      TradingStatus: "on", // Condition from the user collection
