            ...
            if (Number(Panel_key[0].licenses) >= Number(TotalLicense) + Number(req.licence1)) {
                ...
                try {
                    ...
                    if (user_group_service.length == 0) {
                        ...
                        var strategFind = await strategy_client.find({ user_id: existingUsername._id })
                        var client_servicesDelete = await client_services.deleteMany({ user_id: existingUsername._id })
                ...
                return res.send({ status: false, msg: 'You Dont Have License', data: [] });
            }
