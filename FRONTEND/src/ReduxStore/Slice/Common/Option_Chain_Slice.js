...
export const Option_Symbols_Update_status = createAsyncThunk("/update/option_symbols_status", async (apireq) => {

    const { req, token } = apireq

    try {
        const res = await OPTION_SYMBOLS_UPDATE_STATUS({ symbol: req }, token);
        return await res;
    } catch (err) {
        return err;
    }
...
const OptionChainSlice = createSlice({
    ...
    extraReducers: {
        ...
        [Option_Symbols_Update_status.fulfilled]: (state, { payload }) => {
            return { ...state, update_symbol_status: payload, isLoading: false };
        },
    }
})
