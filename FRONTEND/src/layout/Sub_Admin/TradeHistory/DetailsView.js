...
import BasicDataTable from "../../../Components/ExtraComponents/Datatable/BasicDataTable";
import { fa_time, fDateTimeSuffix } from "../../../Utils/Date_formet";
import { GetSignalsAdmins } from "../../../ReduxStore/Slice/Admin/TradehistorySlice";
...
const DetailsView = ({ showModal, setshowModal, tradeHistoryData }) => {
    ...
    const GetSignalsAdmin = async (data) => {
      await dispatch(GetSignalsAdmins({ signal_id: data }))
        .unwrap()
        .then((response) => {
          if (response.status) {
            setSignalsData({ loading: true, result: response.data });
          } else {
            setSignalsData({ loading: false, result: [] });
          }
        })
        .catch((err) => {});
    };
    ...
    const columns1 = [
  ...
  
      {
        dataField: "",
        text: "Ft Time ",
        formatter: (cell, row, rowIndex) => <div>{ft_time(row)}</div>,
      },
    ];
        ...
        >
          {signalsData.loading ? (
            <BasicDataTable
              ...
              tableData={signalsData.result}
            />
          ) : (
          //   ADD Loader witrh spinner here 
              <div className="text-center">Loading...</div>
               
  
              
          )}
          {/* <BasicDataTable TableColumns={columns1} tableData={signalsData} /> */}
        </Modal>
      </div>
    );
  };
