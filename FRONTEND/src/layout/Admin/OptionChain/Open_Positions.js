...
const TradeHistory = () => {
  ...
  const Get_Position = async (e) => {
    await dispatch(Get_Open_Position({ token: token }))
      .unwrap()
      .then((response) => {
        if (response.status) {
          const filterData =
            ...
            response.data.filter((item) => {
              const searchedMatch =
                searchInput === "" ||
                item.TradeType.toLowerCase().includes(
  ...
  const columns = [
...

    {
      ...
      formatter: (cell, row, rowIndex) => (
        <span className={``}>
          {row.entry_type ? row.entry_type : row.exit_type}
        </span>
      ),
    },
    {
      ...
      formatter: (cell, row, rowIndex) => (
        <div className="col-12">
          <input
            ...
            name="exit_time"
            defaultValue={
              row.exit_time.substring(0, 2) + ":" + row.exit_time.substring(2)
            }
            onChange={(e) =>
              SetStopLostPrice(
                e,
  ...
  const UpdateStopLoss = async () => {
    const filteredArray2 = tradeHistoryAllData.data.filter((item) =>
      selected1.some((obj) => obj._id === item._id)
    );
        ...
        setCreateSignalRequest((prev) => {
          return prev.map((item) => {
            if (item.trade_symbol === symbol) {
              // Assuming 'symbol' is the unique identifier
              return {
                ...item,
  ...
  const Done_For_Trade = () => {
    ...
    let count = 0;
      ...
      CreateSignalRequest?.map((pre_tag) => {
        count++;

        if (pre_tag?.new_qty_persent > pre_tag?.old_qty_persent) {
          alert(
            "Error: Value cannot be greater than " + pre_tag?.old_qty_persent
          );
          return;
        }
        ...
        selected1.map((rowdata) => {
          ...
          var pre_tag = {
            client_persnal_key: rowdata.client_persnal_key
              ? rowdata.client_persnal_key
              : PanelKey && PanelKey.client_key,
            TradeType: rowdata.TradeType,
            option_type: rowdata.option_type,
            type:
              rowdata.entry_type === "LE"
                ? "LX"
                : rowdata.entry_type === "SE"
                ? "SX"
                : "",
            trade_symbol: `${rowdata.symbol}${show_expiry}${rowdata.strike}${
              rowdata.option_type === "CALL"
                ? "CE"
                : rowdata.option_type === "PUT"
                ? "PE"
                : ""
            }`,
            ...
            indexcallput:
              rowdata.option_type === "CALL"
                ? `${rowdata.option_type}_${rowdata.token}`
                : `${rowdata.option_type}_${rowdata.token}`,
            ...
            old_qty_persent:
              rowdata.entry_qty_percent && rowdata.exit_qty_percent
                ? parseInt(rowdata.entry_qty_percent) -
                  parseInt(rowdata.exit_qty_percent)
                : rowdata.entry_qty_percent
                ? rowdata.entry_qty_percent
                : rowdata.exit_qty_percent,
            new_qty_persent:
              rowdata.entry_qty_percent && rowdata.exit_qty_percent
                ? parseInt(rowdata.entry_qty_percent) -
                  parseInt(rowdata.exit_qty_percent)
            ...
            setCreateSignalRequest((oldValues) => {
              return oldValues.filter(
                (item) =>
                  item.indexcallput !==
                  (rowdata.option_type === "CALL"
                    ? `${rowdata.option_type}_${rowdata._id}`
                    : `${rowdata.option_type}_${rowdata._id}`)
              );
            });
  ...
  const forMCXandCurrencyMarketTrade = () => {
    ...
    if (MarketOpenToday) {
      ...
      const forEquity =
        new Date(currentDateIST).getTime() > EQcutoffTimeIST.getTime();

      const CUcutoffTimeIST = new Date();
      CUcutoffTimeIST.setHours(17, 0, 0, 0);
      const forCurrency =
        new Date(currentDateIST).getTime() > CUcutoffTimeIST.getTime();

      const MCXcutoffTimeIST = new Date();
      MCXcutoffTimeIST.setHours(23, 30, 0, 0);
      const forMCX =
        new Date(currentDateIST).getTime() > MCXcutoffTimeIST.getTime();
    ...
    <>
      <Content Page_title="Open Position" button_status={false}>
        <div className="row d-flex align-items-center text-secondary mt-4">
          ...
          <button
            className="col-lg-2  btn btn-primary me-3 mt-1 "
            onClick={(e) => UpdateStopLoss()}
          >
            Update Price
          </button>
          <button
            className=" col-lg-2   btn btn-primary mt-1"
                    ...
                    formatter: (cell, row, rowIndex) => (
                      <div>
                        <input
                          ...
                          defaultValue={
                            inputValue ? inputValue : row.old_qty_persent
                          }
