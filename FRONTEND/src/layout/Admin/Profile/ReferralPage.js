...
const ReferralPage = () => {
  ...
  let iframeUrl = Config.react_domain + "/#/newsignup";
  ...
  const [getReferalUsers, setReferalUsers] = useState({
    loading: true,
    data: [],
  });
  const [getReferalUsersData, setReferalUsersData] = useState({
    loading: true,
    data: [],
  });
  ...
  const formik = useFormik({
    initialValues: {
      companyname: getCompanyName.data.length
        ? getCompanyName.data[0].panel_name
        : "",
      refer_points: getCompanyName.data.length
        ? getCompanyName.data[0].refer_points
        : "",
    },
    validate: (values) => {
      const errors = {};
    ...
    onSubmit: async (values) => {
      ...
      await dispatch(Update_smtp_details({ req, token: user_details.token }))
        .unwrap()
        .then((response) => {
        ...
        if (response.status) {
          const filteredData = searchTerm1
            ? response.data.filter((user) =>
                user.UserName.toLowerCase().includes(searchTerm1.toLowerCase())
              )
            : response.data;
  ...
  useEffect(() => {
    if (getCompanyName.data && getCompanyName.data[0]) {
      formik.setFieldValue(
        "companyname",
        getCompanyName.data.length > 0 && getCompanyName.data[0].panel_name
      );
      formik.setFieldValue(
        "refer_points",
        getCompanyName.data.length > 0 && getCompanyName.data[0].refer_points
      );
    }
  }, [getCompanyName.data]);

  const totalReferrals = getReferalUsers.data && getReferalUsers.data.length;
  const inProcessReferrals =
    getReferalUsers.data &&
    getReferalUsers.data.filter((user) => Number(user.ActiveStatus) === 1).length;
  const successfulReferrals =
    getReferalUsers.data &&
    getReferalUsers.data.filter((user) => Number(user.ActiveStatus) === 2).length;
//   const ReferralsPoints = getReferalUsers.data
//     ? getReferalUsers.data
//         .filter((user) => Number(user.ActiveStatus) === 0)
  ...
  const Payment = async (status, data) => {
    await dispatch(
      UPDATE_REEDEEM({
        ...
        id: data._id,
        reedeem_points: data.reedeem_points,
      })
    )
      .unwrap()
      .then((response) => {
              ...
              <div className="card-body">
                              ...
                              <div className="col-md-4">
                                <div className="rpWrp3">
                                  <h3>
                                    ...
                                    <p className="mb-0">
                                      {inProcessReferrals && inProcessReferrals}
                                    </p>
                                  </h3>
                                </div>
                              </div>
                ...
                >
                  <Tab eventKey="home" title="Refer Information">
                    ...
                    <BasicDataTable
                      tableData={getReferalUsers.data}
                  ...
                  <Tab eventKey="profile" title="Reedeem Request">
                    ...
                    <BasicDataTable
                      tableData={getReferalUsersData.data}
                ...
                <div className="iframe-container mb-3 d-flex align-items-center">
                  ...
                  <FaSms
                    size={32}
                    onClick={() => shareUrl("sms")}
                    style={{ cursor: "pointer", marginLeft: "10px" }}
                  />
                  <FaEnvelope
                    size={32}
                    onClick={() => shareUrl("email")}
                    style={{ cursor: "pointer", marginLeft: "10px" }}
                  />
                  <FaInstagram
                    size={32}
