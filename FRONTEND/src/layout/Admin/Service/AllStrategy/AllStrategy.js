...
const ServicesList = () => {
  ...
  const handleStrategyChecked = (event) => {
    ...
    if (event.target.checked) {
      ...
      setStartegyClientIdsDelete(
        startegyClientIdsDelete.filter((item) => item !== CheckedClientId)
      );
    } else {
      setStartegyClientIdsDelete([...startegyClientIdsDelete, CheckedClientId]);
      setStartegyClientIds(
        startegyClientIds.filter((item) => item !== CheckedClientId)
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let filteredArray = startegyClientIds.filter(
      (item) => !oneStrategyClient.includes(item)
    );
    ...
    await dispatch(UpDate_strategy_Id({ req: req }))
      .unwrap()
      .then((response) => {
        if (response.status) {
          toast.success(response.msg);
              ...
              <tbody>
                {getClient &&
                  getClient.map((client, index) => {
                    return (
                      <tr className="mx-3 h4 font-weight-bold">
                        <td>
                          <input
                            type="checkbox"
                            id={client._id}
                            name={client._id}
                            value="checkboxValue"
                            defaultChecked={startegyClientIds.includes(
                              client._id
                            )}
                          ...
                          <label className="mx-3" htmlFor={client._id}>
                            {client.UserName}
                          </label>
                          {oneStrategyClient.includes(client._id) ? (
                            <Link
                              to={`/admin/client/edit/${client._id}`}
                              state={client}
                            >
                              <span
                                data-toggle="tooltip"
