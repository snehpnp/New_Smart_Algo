...
const TradeHistory = () => {
    ...
    const getsignals = async (e) => {
        ...
        await dispatch(Get_Tradehisotry({ user_id: user_id, startDate: startDate, endDate: endDate, token: token })).unwrap()
            .then((response) => {
                if (response.status) {
                    setTradeHistoryData({
                        loading: false,
    ...
    const getsignals11 = async (e) => {
        ...
        await dispatch(Get_Tradehisotry({ user_id: user_id, startDate: full, endDate: full, token: token })).unwrap()
            .then((response) => {
                if (response.status) {
                    setTradeHistoryData({
                        loading: false,
